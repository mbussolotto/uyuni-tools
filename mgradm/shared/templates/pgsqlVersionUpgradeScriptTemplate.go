...
OLDVERSIONDIR="/var/lib/pgsql/data-pg$OLD_VERSION"
test -d "$ODLVERSIONDIR" && mv "$OLDVERSIONDIR" "${OLDVERSIONDIR}-${RANDOM}"
mkdir -p "$OLDVERSIONDIR"
echo "Create a backup at $OLDVERSIONDIR"
mv /var/lib/pgsql/data/* /var/lib/pgsql/$OLDVERSIONDIR
...
su -s /bin/bash - postgres -c "pg_upgrade --old-bindir=/usr/lib/postgresql$OLD_VERSION/bin --new-bindir=/usr/lib/postgresql$NEW_VERSION/bin --old-datadir=$OLDVERSIONDIR --new-datadir=/var/lib/pgsql/data $FAST_UPGRADE"

cp $OLDVERSIONDIR/pg_hba.conf /var/lib/pgsql/data
mv $OLDVERSIONDIR/pg_hba.conf $OLDVERSIONDIR/pg_hba.conf.migrated
cp $OLDVERSIONDIR/postgresql.conf /var/lib/pgsql/data/
mv $OLDVERSIONDIR/postgresql.conf $OLDVERSIONDIR/postgresql.conf.migrated
