...
func restoreSystemdFile(header *tar.Header, tr *tar.Reader) error {
	// Systemd backup does not package directories anymore, so be sure they are present
...
func generateDefaltSystemdServices(flags *shared.Flagpole) error {
	...
	// Generate minimum set uyuni-server services - like we do on default install
	...
	return utils.JoinErrors(
		podman.GenerateSystemdService("", serverImage, false, "", []string{}),
