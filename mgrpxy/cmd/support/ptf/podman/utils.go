...
func ptfForPodman(
	...
	if err := podman_shared.StopService(podman_shared.ProxyService); err != nil {
		return err
	}
	...
	err = podman.GenerateSystemdService(httpdImage, saltBrokerImage, squidImage, sshImage,
		tftpdImage, &flags.UpgradeFlags)
	if err != nil {
		return err
	}

	return podman.StartPod()
}
...
func updateParameters(flags *podmanPTFFlags) error {
	...
	for _, config := range proxyImages {
		...
		config.imageFlag.Name, err = utils.ComputePTF(flags.UpgradeFlags.ProxyImageFlags.Registry.Host,
